...
const Works = () => {
  ...
  function getLocalData(new_category_idx) {
    ...
    works_data
      .getWorksbyCategory(categories[new_category_idx])
      .then((data) => {
  ...
  return (
    <div className="app__works">
        ...
        <div className="app__works-categoryBox">
          ...
          >
            {categories.map((category_item, index) => (
              <li className="p-text p-link" key={`category-${index}`} onClick={() => getLocalData(index)}>
      ...
      <div className="app__container section_pad">
        ...
        {loading ? (
          <img alt="loading" src={images.loading} className="app__flex app__loading" />
        ) : (
          <>
            <ul className="app__works-portfolio">
              {portfolioItems.map((port_item, index) => (
                <li key={`work-${index}`} className="app__cardcover">
                  ...
                  <img src={port_item["logo"]} alt={`mohammad-amin-rashid-${port_item["title"]}`} />
                </li>
              ))}
            </ul>
          </>
